...
## Tech stack
- Next.js (App Router)
- Deployed on Vercel via GitHub repo: nikbearbrown/livingmodels_org
...
8. `/dev/[...slug]` — Full-viewport iframe of a dev doc HTML file (e.g. `/dev/Living Models/doc-name`)
...
- Logo: text link "Living Models" linking to `/`
...
### Adding new dev docs
1. Create a subdirectory in `public/dev/` (e.g. `public/dev/Living Models/`)
...
- `/dev/[...slug]` — full-viewport iframe of the doc (e.g. `/dev/Living Models/doc-name`)
...
## Theming (Living Models template)
...
NEXT_PUBLIC_SITE_URL=https://livingmodels.org  # Used in sitemap generation
...
livingmodels.org is the Living Models adaptive learning platform site — part product intro, part tool directory, part blog. It runs on Next.js with Vercel auto-deploy. The public site has no login; the admin dashboard is cookie-protected.
...
NEXT_PUBLIC_SITE_URL=https://livingmodels.org
```
