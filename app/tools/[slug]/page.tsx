...
export async function generateMetadata({
  ...
  if (doc) {
    return {
      title: `${doc.title} - Living Models Tools`,
      description: doc.description || `${doc.title} — AI tool by Living Models`,
    }
  }
  ...
  try {
    ...
    if (rows.length > 0) {
      return {
        title: `${rows[0].name} - Living Models Tools`,
        description: rows[0].description || `${rows[0].name} — AI tool by Living Models`,
      }
    }
  } catch {}

  return { title: 'Tool - Living Models' }
}
